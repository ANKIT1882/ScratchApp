...
export default function CatSprite({ spriteData,x,y }) {
  return (
    <div  
     className={`flex ${spriteData.corner?'animate-bounce':''}`}
    style={{
      marginLeft:`${spriteData.position.x}px`,
      marginTop:`${spriteData.position.y}px`,
      transform: `rotate(${spriteData.direction}deg)`,
      height:'fit-content',
      width:'fit-content',
      scale:`${parseInt(spriteData.size)/100}`
    }}>
     <div className="flex-col">
     <div
        className="w-20 h-20 text-center ml-10"
        style={{
          background: `url('/dialog.svg')`,
          backgroundSize: "100px 100px",
          backgroundPosition: "center",
          visibility:spriteData.thinkFlag?'visible':'hidden'
        }}
      >
        <p className="top-2/4" style={{ transform: "translateY(50%)" }}>
          {spriteData.thinkWord}
        </p>
      </div>
      <svg
          ...
          <g id="costume1.1">
            <g id="tail">
              <path
                ...
                stroke="#001026"
                strokeWidth="1.2"
                fill={`hsl(${38 + parseInt(spriteData.color)},100%,54.9%)`}
                strokeLinecap="round"
      ...
      <p style={{visibility:x?'visible':'hidden'}}>x:{spriteData.position.x.toFixed(1)}</p>
      <p style={{visibility:y?'visible':'hidden'}}>y:{spriteData.position.y.toFixed(1)}</p>
     </div>
      ...
      <div
        ...
        style={{
         ...
         visibility:spriteData.sayFlag?'visible':'hidden'
        }}
      >
        <p className="top-2/4" style={{ transform: "translateY(50%)" }}>
          {spriteData.sayWord}
